...
@Component({
  ...
  export class  PrevisualizarClaseComponent implements OnInit{
    form!: FormGroup;
    clase:string=''
    constructor(private http: HttpClient, private toastr: ToastrService, public dialog: MatDialog, private jwtService:JwtService, private router:Router) {
    ...
    nav_verclase(clase:string){
      this.router.navigate([`/profesor/${this.idProfesor}/ver-clase/${clase}`]);
    }
