...
var Puntos = 0;
@Component({
  selector:  'app-PCMatematicas',
  templateUrl: './PCMatematicas.component.html',
  styleUrls: ['./VerClase.component.scss', '../../FondoP.component.scss']
})
export class PCMatematicasComponent {
    isDisabled = false;
 ...
 xpTot: number=0;
    validacion(solucion: string, result:InnerHTML){
        ...
        }else if(alumnoVoz.toUpperCase().includes(solucion.toUpperCase())){
            Puntos++;
            console.log("Correcto"+Puntos);
            result.innerHTML = 'Correcto, sigue así.<br> +'+xpBien+"xp";
            this.xpTot=this.xpTot+xpBien;
